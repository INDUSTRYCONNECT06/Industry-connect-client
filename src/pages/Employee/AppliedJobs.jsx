          ...
          <section className="w-full flex flex-col gap-8">
            {jobsData?.data &&
              jobsData?.data?.map((element) => {
                return <JobCard jobData={element?.jobId} orgData={element?.jobId?.employerId} key={element?._id} appliedButton={false} bookmarkedButton={false} appliedAt={element?.createdAt}/>;
              })}
          </section>
        </div>
      </section>
